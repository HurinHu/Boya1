    ...
    <script>
		KindEditor.ready(function(K) {
			var editor1 = K.create('textarea[name="content1"]', {
				...
				uploadJson : '<?=base_url('/service/upload')?>',
				fileManagerJson : '<?=base_url('/service/uploadfile')?>',
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1>首页管理</h1>
	       <div>
	       		...
	       		<form class="form-horizontal" role="form" name="example" action="<?=base_url('/service/updatearticle/'.$id)?>" method="post" enctype="multipart/form-data">
				    ...
				    <div class="form-group">
					    <label for="introduction" class="col-sm-2 control-label">预览文字(150字内)</label>
					    <div class="col-sm-10">
						    <textarea class="form-control" id="introduction" name="introduction" rows="3"><?=$introduction;?></textarea>
						</div>
					</div>
				    <div class="form-group">
			       		<label for="content1" class="col-sm-2 control-label">内容</label>
			       		<div class="col-sm-6">
				       		<textarea name="content1" id="content1" style="width:700px;height:500px;visibility:hidden;"><?=htmlspecialchars($content);?></textarea>
				       	</div>
				    </div>

					<p class="text-primary">如有缓存，请点击刷新按钮或按F5</p>
					<div class="form-group">
						<div class="col-sm-offset-2 col-sm-10">
							<button type="submit" class="btn btn-primary">更新</button>&nbsp;<a class="btn btn-primary" href="<?=base_url('/service');?>">返回</a> 
						</div>
					</div>
				</form>
	       </div>
